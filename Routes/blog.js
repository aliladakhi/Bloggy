...
.post(upload.single('profilepic'),async(req,res)=>{
  ...
  const blog=await Blog.create({title,coverUrl,blogContent,owner:req.user.id})
...
.get(async (req,res)=>{
  ...
  const {title,blogContent}=await Blog.findOne({_id:id})
...
console.log(commentss);
  ...
  res.render("POST",{name:req.user.name,title,blogContent,LikeUsers,commentss})
})

blogRouter.route('/like/:id')
.post( async (req, res) => {
  try {
